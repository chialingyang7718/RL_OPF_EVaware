...
class PowerGrid(Env):
    def __init__(
        ...
        self.action_space = Box(
            low=np.full((2 * self.NsG + self.N_EV,), -1),
        ...
        self.observation_space = Box(
            low=np.zeros((2 * self.NL + self.NsG + self.N_EV,)),
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.EVaware == True:
            ...
            self.state[2 * self.NL + self.NsG :] = self.net.storage.loc[
                :, "soc_percent"
            ]
    ...
    def define_limit(self, time_step):
        ...
        if "max_p_mw" not in self.net.sgen:
            self.net.sgen.loc[:, "max_p_mw"] = (
                self.net.sgen.loc[:, "p_mw"] * 1.5
            )  # assume the max power output is 1.5 times the current power output if not specified
        if "max_q_mvar" not in self.net.sgen:
            self.net.sgen.loc[:, "max_q_mvar"] = (
                abs(self.net.sgen.loc[:, "q_mvar"]) * 1.5
            )  # assume the max reactive power output is 1.5 times the current reactive power output if not specified
        if "min_q_mvar" not in self.net.sgen:
            self.net.sgen.loc[:, "min_q_mvar"] = (
                abs(self.net.sgen.loc[:, "q_mvar"]) * -1.5
            )  # assume the min power output is 1.5 times the current power output if not specified
        self.PsGmax = np.full(
            self.NsG, self.net.sgen.loc[:, "max_p_mw"].max()
        )  # to avoid limitation of zero if any original value is zero
        self.PsGmin = np.zeros(
            self.NsG
        ...
        self.PsGcap = self.mu_renewable.flatten("F").astype(
            np.float64
        )  # set the max power generation for action space

        # assign the load limits (PQ bus)
        self.PLmax = np.full(
            self.NL, self.net.load.loc[:, "p_mw"].max() * 1.5
        )  # ASSUMPTION: the max active power consumption is 1.5 times the maximum value in the net
        self.PLmin = np.zeros(
            self.NL
        )  # ASSUMPTION: the min active power consumption is 0 MW
        self.QLmax = np.full(
            self.NL, abs(self.net.load.loc[:, "q_mvar"]).max() * 1.5
        )  # ASSUMPTION: the max reactive power consumption is 1.5 times the maximum value in the net
        self.QLmin = (
            -self.QLmax
    ...
    def update_EV_limit(self, time_step):
        ...
        for i in range(self.N_EV):
            ...
            self.PEVmax[i] = min(
                0.001 * 150 * self.net.storage.loc[i, "n_car"], charging_max[i]
            )
            self.PEVmin[i] = -min(
                0.001 * 150 * self.net.storage.loc[i, "n_car"], discharge_max[i]
            )
    ...
    def update_EV_state(self, time_step):
        ...
        for i in range(self.N_EV):
            ...
            if (
                self.net.res_storage.loc[i, "p_mw"] >= 0
            ):  # charging with time step 1 hour
                ...
                if energy_afterdischarging >= 0:
                    ...
                    print("Negative SOC!")
            self.state[self.NL * 2 + self.NsG + i] = self.net.storage.loc[
                i, "soc_percent"
            ]
    ...
    def _get_observation(self):
        ...
        for i in range(self.NsG):
            normalized_obs.append(
                self.normalize(
                    self.state[i + 2 * self.NL], self.PsGmax[i], self.PsGmin[i], 1, 0
                )
            )
    ...
    def apply_action_to_net(self, action):
        for i in range(self.NsG):  # static generator (PQ bus)
            denormalized_p = self.denormalize(
                action[i], self.PsGcap[i], self.PsGmin[i], 1, -1
            )
            ...
            self.net.sgen.loc[i, "q_mvar"] = self.denormalize(
                action[i + self.NsG], self.QsGmax[i], self.QsGmin[i], 1, -1
            )

            for i in range(self.N_EV):
                denormalized_pEV = self.denormalize(
                    action[i + 2 * self.NsG], self.PEVmax[i], self.PEVmin[i], 1, -1
                )
                ...
                if denormalized_pEV < 0:
                    ...
                    self.net.storage.loc[i, "p_mw"] = (
                        discharge_efficiency * denormalized_pEV
                    )
                else:
                    # charging
        ...
        if violated_buses.size != 0:
            ...
            for violated_bus in violated_buses:
                if self.net.res_bus.loc[violated_bus, "vm_pu"] < self.VGmin:
                    penalty_voltage += (
                        self.net.res_bus.loc[violated_bus, "vm_pu"] - self.VGmin
                    ) * 1000
                else:
                    penalty_voltage += (
                        self.VGmax - self.net.res_bus.loc[violated_bus, "vm_pu"]
                    ) * 1000
    ...
    def calculate_gen_cost(self):
        ...
        if self.net.poly_cost.index.size > 0:
            for i in self.net.sgen.index:
                gen_cost += (
                    self.net.res_sgen.p_mw[i] ** 2 * self.net.poly_cost.iat[i, 4]
                    + self.net.res_sgen.p_mw[i] * self.net.poly_cost.iat[i, 3]
                    + self.net.poly_cost.iat[i, 2]
                    + self.net.poly_cost.iat[i, 5]
                    + self.net.poly_cost.iat[i, 6] * self.net.res_sgen.q_mvar[i]
                    + self.net.poly_cost.iat[i, 7] * self.net.res_sgen.q_mvar[i] ** 2
                )
        ...
        # if the cost function is missing
            ...
            total_gen = self.net.res_sgen[
                "p_mw"
            ].sum()  # + self.net.res_ext_grid['p_mw'].sum()
