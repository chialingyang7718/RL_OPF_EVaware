...
- The active and reactive power of loads, maximum active power generation are uniformly distributed between their min and max values.
...
- The maximum EV (dis)charging power is 3.7 kW.
...
- States: load active power, load reactive power, maximum active power generation, EV SOC, EV connection status
"""
...
class PowerGrid(Env):
    def __init__(self, net, dispatching_intervals=24, Training=False):  # assuming dispatching intervals = number of hours in a day
        ...
        # define the observation space: PL, QL, P_renewable(max generation from renewable energy sources), SOC_EV, connetion status
    ...
    def step(self, action):
        ...
        try:
            pp.runpp(self.net) # Newton-Raphson method
            # pp.runpp(self.net, algorithm='gs') # Gauss-Seidel method

        except:
            print("Power flow does not converge!")
            # print(pp.diagnostic(self.net, report_style="detailed")) # output the diagnostic report
            # assign the reward in the case of the power flow does not converge
            reward = -5000
            diverged = True
        else:
            diverged = False
            reward, violated_buses, overload_lines, violated_phase = (self.calculate_reward())
    ...
    def reset(self, seed=None, options=None):
        ...
        # initialize the time step and SOC
    ...
    def define_limit(self):
        ...
        self.Vmax = (self.net.bus.loc[:, "max_vm_pu"].to_numpy().flatten("F").astype(np.float64))
        self.Vmin = (self.net.bus.loc[:, "min_vm_pu"].to_numpy().flatten("F").astype(np.float64))
        ...
        self.linemax = (self.net.line.loc[:, "max_loading_percent"].to_numpy().flatten("F").astype(np.float64))
    ...
    def update_EV_limit(self):
        ...
        for i in range(self.N_EV):
            ...
            EV_power_demand = (
                ...
                * self.net.storage.loc[i, "n_car"] / 1000
            ...
            # the max. charging speed is 3.7 kw and assume all the EVs are connected in parallel
    ...
    def create_state(self):
        """
        This funtion is served as the following purposes:
        1. read the active and reactive power of the loads, active power of gen from self.net.load
        2. randomly select SOC of the EVs under defined EV scenario as the initial SOC
        3. fetch EV power demand from the EV profile at the selected time step
        4. update the connection state of the EVs
        5. create the initial state by concatenating
        """
    ...
    def load_EV_spec_profiles(self):
        """
        This function is to preprocess the EV spec and EV profile data.
        The function creates df_EV_spec and df_EV where df_EV is a two-level indexed DataFrame with the ID and Time_step as the indices in which only relevant EV IDs are extracted.
        Notice that all days are included in the df_EV. Thus, the total time steps are 365*24 = 8760.
        """
        # create the EV spec DataFrame
        rows_to_read = [
            0,
            1069,
            1070,
        ...
        # create the EV profile DataFrame
        ...
        def convert_to_float_int(value):
            """
            Since the data is read from a CSV file, all values are read as strings. 
            Thus, this function is to convert the values to float or int if possible.
            """
            try:
    ...
    def add_EV_group(self, grid):
        for i in grid.load.index:
            ...
            n_car = int(grid.load.loc[i, "p_mw"]) 
            if n_car == 0:
    ...
    def apply_action_to_net(self, action):
        for i in range(self.NG):
            ...
            self.net.gen.loc[i, "vm_pu"] = self.denormalize(action[i + self.NG], self.VGmax[i], self.VGmin[i], 1, -1)
        ...
        if violated_buses.size != 0:
            ...
            for violated_bus in violated_buses:
                if self.net.res_bus.loc[violated_bus, "vm_pu"] < self.Vmin[violated_bus]:
                    penalty_voltage += (self.net.res_bus.loc[violated_bus, "vm_pu"] - self.Vmin[violated_bus]) * 1000
                else:
                    penalty_voltage += (self.Vmax[violated_bus] - self.net.res_bus.loc[violated_bus, "vm_pu"]) * 1000

        # line overload violation
        if overload_lines.size != 0:
            self.violation = True
            for overload_line in overload_lines:
                penalty_line += (self.linemax[overload_line] - self.net.res_line.loc[overload_line, "loading_percent"]) * 10
